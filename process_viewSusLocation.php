...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    if (isset($_GET["caseID"])) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $page_valid = isset($_POST['page_valid']) ? $_POST['page_valid'] : 1;
                $geoID = $row['caseGeoID'];
                $lat = $row['latitude'];
                $lng = $row['longitude'];
                $pname = $row['pname'];
                ...
                $markerCode .= "marker$geoID.bindPopup('Name: $pname<br>Barangay: $barangay <br><form method=\"POST\" action=\"process_updateSusLocation.php\"><input type=\"hidden\" name=\"page_valid\" value=\"$page_valid\"><input type=\"hidden\" name=\"caseID\" value=\"$caseID\"><button type=\"submit\" name=\"geoID\" value=\"$geoID\" class=\"btn btn-primary\">Update Location</button></form>').openPopup();\n";
            }
            // echo $markerCode;
        }
    } else {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $markerCode .= "var marker$geoID = L.marker([$lat, $lng]).addTo(map);\n";
            $markerCode .= "marker$geoID.bindPopup('Name: $pname<br>Barangay: $barangay <br><form method=\"POST\" action=\"process_updateSusLocation.php\"><input type=\"hidden\" name=\"caseID\" value=\"$caseID\"><input type=\"hidden\" name=\"page_valid\" value=\"$page_valid\"><button type=\"submit\" name=\"geoID\" value=\"$geoID\" class=\"btn btn-primary\">Update Location</button></form>').openPopup();\n";
        }
        // echo $markerCode;
    }
} else {
...
<head>
    ...
    <title>Suspected Location</title>
    ...
    <style>
        ...
        .main-content {
            ...
            /* Subtract padding from container height */
        }

        .col {
            /* Keep other styles as they are */
            height: 100%;
            /* Set column height to 100% of its parent (main-content) */
        }
                    ...
                    <div class="col-md-8 p-1 mt-2 my-auto mx-auto">
                        <div class="container mt-2 p-3">
                            <div class="col-md-auto">
                                <a href="./dashboardRabidCases.php?active-tab=2&page_valid=<?php echo ($page_valid); ?>" class="btn btn-lg align-item-start">
                                    <i class="bi bi-arrow-left"></i>
                                </a>
                            </div>
                            <!-- <form method="POST" action="./dashboardRabidCases.php?active-tab=1" id="heatmapPet">
                <button type="submit" class="btn btn-primary btn-lg">Back</button>
    </form> -->
                            <div>
                                <h2 class="mt-3 mb-4">Suspected Case Location:</h2>
                                <div id="map"></div>
