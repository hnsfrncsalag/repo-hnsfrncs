...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = filterData($barangay, $petType, $vaccination, $min_date, $max_date);
    ...
    $count2 = new Pet();
    $count3 = new Pet();
    $counts = $count->countPet($barangay);
    $counts1 = $count1->countAllPet($barangay);
    $counts2 = $count2->countAllPetPerYear($barangay);
    $counts3 = $count3->countAllValidPetPerYear($barangay);
    $pets = $pet->getRegistries($barangay);
}

function filterData($barangay, $petType, $vaccination, $min_date, $max_date)
{
    // $query = "SELECT * FROM `pet` p ON c.petID = p.petID LEFT JOIN `vaccination` v ON p.petID = v.petID LEFT JOIN `geolocation` g ON g.geoID = c.caseGeoID LEFT JOIN `barangay` b ON b.brgyID = c.brgyID WHERE 1"; // Start with 1 to ensure the WHERE clause is always valid

    $query = "SELECT `P`.*, `R`.*, `V`.*, `P`.*, `G`.*, `B`.*
    FROM `pet` AS `P` 
        LEFT JOIN `resident` AS `R` ON `P`.`residentID` = `R`.`residentID` 
        LEFT JOIN `vaccination` AS `V` ON `V`.`petID` = `P`.`petID` 
    ...
    $conditions = [];
    if (!empty($barangay)) {
        $conditions[] = "R.brgyID = '{$barangay}'";
    }
    if (!empty($petType)) {
        $conditions[] = "P.petType = '{$petType}'";
    }
    if (!empty($vaccination)) {
        $conditions[] = "P.statusVac = '{$vaccination}'";
    }
    if (!empty($min_date) && !empty($max_date)) {
        $conditions[] = "P.currentVac BETWEEN '{$min_date}' AND '{$max_date}'";
    }
            ...
            <div class="col d-flex flex-column h-sm-100">
                ...
                <h5 class="card-title">Charts</h5> -->
                            ...
                            <!-- <div class="table-responsive">
                                <table id="registries" class="table text-center table-striped table-bordered table-hover">
                                    <div class="row">
                                        <thead>
