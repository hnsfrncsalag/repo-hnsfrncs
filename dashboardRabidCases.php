    ...
    if (isset($_POST["accept"]) || isset($_POST["reject"])) {
        ...
        if ($result === true) {
            ...
            if ($caseStatus == 2 && $notificationResult === true) {
                ...
                echo '<script>alert("Case Status Updated Successfully.")window.location.href = "./dashboardRabidCases.php?active-tab=1";</script>';
            } else {
                // Redirect as per your existing logic
                echo '<script>alert("Case Status Updated Successfully."); window.location.href = "process_viewSusLocation.php?caseID=' . $caseID . '";</script>';
            }
        } else {
...
<head>
    ...
    <title>Manage Suspected Case</title>
                ...
                <main class="row overflow-auto">
                    <div class="col-md-10 p-1 mt-2 my-auto mx-auto">
                        <div class="container mt-4">
                            <h1 class="text-center">Manage Suspected Cases for Barangay: <?php echo $result1 ?></h1>
                            <!-- Navigation Tabs -->
                            <ul class="nav nav-tabs">
                                <li class="nav-item">
                                    <a class="nav-link<?= ($active_tab == 1) ? ' active' : '' ?>" href="./dashboardRabidCases.php?active-tab=1&page_new=1#newRabid" data-bs-toggle="tab">New Reports</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link<?= ($active_tab == 2) ? ' active' : '' ?>" href="./dashboardRabidCases.php?active-tab=2&page_valid=1#validRabid" data-bs-toggle="tab">Valid Reports</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link<?= ($active_tab == 3) ? ' active' : '' ?>" href="./dashboardRabidCases.php?active-tab=3&page_rej=1#rejectedRabid" data-bs-toggle="tab">Rejected Reports</a>
                                </li>
                            </ul>
                            ...
                            <div class="tab-content">
                                ...
                                $newSusQuery = "SELECT c.*, p.*, c.residentID, r.name
                                FROM `case` AS c
                                INNER JOIN pet AS p ON p.petID = c.petID
                                INNER JOIN resident AS r ON c.residentID = r.residentID
                                WHERE  (pname LIKE ? OR victimsName LIKE ? OR name LIKE ?) AND c.caseStatus = 0 AND c.caseType = 2 AND c.brgyID = ? 
                                    AND (r.name IS NOT NULL OR p.pname IS NOT NULL) ORDER BY c.date DESC";

                                // Prepare and execute the SQL query
                                $stmt_new = mysqli_prepare($conn, $newSusQuery);
                                ...
                                $newSusResult = mysqli_stmt_get_result($stmt_new);
                                ...
                                if ($newSusResult && mysqli_num_rows($newSusResult) > 0) {
                                    // Fetch all search results
                                    $allNewSus = mysqli_fetch_all($newSusResult, MYSQLI_ASSOC);

                                    // Calculate pagination parameters for search results
                                    $total_items_new = count($allNewSus);
                                    ...
                                    $newSus = array_slice($allNewSus, $start_index_new, $items_per_page_new);
                                }
                                ?>
                                <div class="tab-pane <?= ($active_tab == 1) ? ' active show' : '' ?>" id="newRabid">
                                    <div class="table">
                                        <div class="card-body">
                                            <label for="newPetSrch" class="form-label"></label>
                                            <form method="POST" action="dashboardRabidCases.php?active-tab=1">
                                            ...
                                            <div class=table-responsive>
                                                <table class="table text-center table-bordered table-hover" style="width:100%">
                                                    ...
                                                    <tbody id=" valid-n">
                                                        ...
                                                        <?php
                                                        // $case = new Cases();
                                                        // $items_per_page_newRabid = 8; // Number of items per page

                                                        // // Get total number of new suspected rabid cases
                                                        // $total_items_newRabid = count($case->getNewSus($brgyID));
                                                        // $total_pages_newRabid = ceil($total_items_newRabid / $items_per_page_newRabid);

                                                        // // Initialize $page_newRabid and set it based on your requirements
                                                        // $page_newRabid = isset($_GET['page_new']) ? $_GET['page_new'] : 1;

                                                        // // Calculate the starting index for the current page
                                                        // $start_index_newRabid = ($page_newRabid - 1) * $items_per_page_newRabid;

                                                        // // Get the data for the current page
                                                        // $newSus = array_slice($case->getNewSus($brgyID), $start_index_newRabid, $items_per_page_newRabid);
                                                        if (empty($newSus)) {
                                                            echo '<tr><td colspan="10">No data found.</td></tr>';
                                                        } else {
                                                            foreach ($newSus as $row) {
                                                                ...
                                                                <form method="post" action="./dashboardRabidCases.php?active-tab=1">
                                                                    ...
                                                                    <input type="hidden" name="notifType" id="notifType" value="13">
                                                                    ...
                                                                    <button type="submit" name="accept" class="btn btn-success my-1" onclick="document.getElementById(\'notifMessage\').value=\'Suspected Rabid Case has been accepted.\'">Accept</button>
                                                                    <button type="submit" name="reject" class="btn btn-danger my-1" onclick="document.getElementById(\'notifMessage\').value=\'Suspected Rabid Case has been rejected.\'">Reject</button>                                                                       
                                                                </form>
                                                                </td>';
                                                                echo '</tr>';
                                                            }
                                                ...
                                                <ul class="pagination">
                                                    <?php for ($n = 1; $n <= $total_pages_new; $n++) { ?>
                                                        <li class="page-item <?= ($n == $page_new) ? 'active' : '' ?>">
                                                            <a class="page-link" href="./dashboardRabidCases.php?active-tab=1&page_new=<?= $n ?>&newSearch=<?php echo urlencode($search_new); ?>"><?= $n ?></a>
                                                        </li>
                                                    <?php } ?>
                                                </ul>
                                            </div>
                                ...
                                // Construct the SQL query to filter data based on the search terms
                                // Modify this query according to your database structure and requirements
                                $validSusQuery = "SELECT c.*, p.*, c.residentID, r.name
                                FROM `case` AS c
                                INNER JOIN pet AS p ON p.petID = c.petID
                                INNER JOIN resident AS r ON c.residentID = r.residentID
                                WHERE  (pname LIKE ? OR victimsName LIKE ? OR name LIKE ?) AND c.caseStatus = 1 AND c.caseType = 2 AND c.brgyID = ? 
                                    AND (r.name IS NOT NULL OR p.pname IS NOT NULL) ORDER BY c.date DESC";

                                // Prepare and execute the SQL query
                                $stmt_valid = mysqli_prepare($conn, $validSusQuery);
                                ...
                                $validSusResult = mysqli_stmt_get_result($stmt_valid);
                                ...
                                // Check if search results are found
                                if ($validSusResult && mysqli_num_rows($validSusResult) > 0) {
                                    // Fetch all search results
                                    $allvalidSus = mysqli_fetch_all($validSusResult, MYSQLI_ASSOC);

                                    // Calculate pagination parameters for search results
                                    $total_items_valid = count($allvalidSus);
                                    ...
                                    // Slice the search results to display the current page
                                    $validSus = array_slice($allvalidSus, $start_index_valid, $items_per_page_valid);
                                }
                                ?>
                                <div class="tab-pane <?= ($active_tab == 2) ? ' active show' : '' ?>" id="validRabid">
                                    <div class="table">
                                        <div class="card-body">
                                            <label for="ValidSrch" class="form-label"></label>
                                            <form method="POST" action="dashboardRabidCases.php?active-tab=2">
                                                <div class="input-group mb-3">
                                            ...
                                            <div class=table-responsive>
                                                <table class="table text-center table-bordered table-hover" style="width:100%">
                                                    ...
                                                    <tbody id="valid-v">
                                                        <?php
                                                        // $case = new Cases();
                                                        // $items_per_page_validRabid = 8; // Number of items per page

                                                        // // Get total number of valid suspected rabid cases
                                                        // $total_items_validRabid = count($case->getValidSus($brgyID));
                                                        // $total_pages_validRabid = ceil($total_items_validRabid / $items_per_page_validRabid);

                                                        // // Initialize $page_validRabid and set it based on your requirements
                                                        // $page_validRabid = isset($_GET['page_valid']) ? $_GET['page_valid'] : 1;

                                                        // // Calculate the starting index for the current page
                                                        // $start_index_validRabid = ($page_validRabid - 1) * $items_per_page_validRabid;

                                                        // // Get the data for the current page
                                                        // $validSus = array_slice($case->getValidSus($brgyID), $start_index_validRabid, $items_per_page_validRabid);
                                                        if (empty($validSus)) {
                                                            echo '<tr><td colspan="10">No data found.</td></tr>';
                                                        } else {
                                                            foreach ($validSus as $row) {
                                                                echo '<tr class="text-center">';
                                                                echo '<td>' . $row['name'] . '</td>';
                                                                echo '<td>' . (!empty($row['pname']) ? $row['pname'] : 'Unknown') . '</td>';
                                                                echo '<td>' . ($row['petType'] == 0 ? 'Dog' : 'Cat') . '</td>';
                                                                ...
                                                                echo '<td>' . $row['description'] . '</td>';
                                                                echo '<td>
                                                                    <form method="post" action="process_viewSusLocation.php">
                                                                        <input type="hidden" name="caseID" value="' . $row['caseID'] . '">
                                                ...
                                                <ul class="pagination">
                                                    <?php for ($n = 1; $n <= $total_pages_valid; $n++) { ?>
                                                        <li class="page-item <?= ($n == $page_valid) ? 'active' : '' ?>">
                                                            <a class="page-link" href="./dashboardRabidCases.php?active-tab=2&page_valid=<?= $n ?>&validSearch=<?php echo urlencode($search_valid); ?>"><?= $n ?></a>
                                                        </li>
                                                    <?php } ?>
                                                </ul>
                                            </div>
                                ...
                                // Construct the SQL query to filter data based on the search terms
                                // Modify this query according to your database structure and requirements
                                $rejSusQuery = "SELECT c.*, p.*, c.residentID, r.name
                                FROM `case` AS c
                                INNER JOIN pet AS p ON p.petID = c.petID
                                INNER JOIN resident AS r ON c.residentID = r.residentID
                                WHERE  (pname LIKE ? OR victimsName LIKE ? OR name LIKE ?) AND c.caseStatus = 2 AND c.caseType = 2 AND c.brgyID = ? 
                                    AND (r.name IS NOT NULL OR p.pname IS NOT NULL) ORDER BY c.date DESC";

                                // Prepare and execute the SQL query
                                $stmt_rej = mysqli_prepare($conn, $rejSusQuery);
                                $search_term = "%$search_valid%"; // Adding wildcards for LIKE search
                                ...
                                $rejSusResult = mysqli_stmt_get_result($stmt_rej);
                                ...
                                // Check if search results are found
                                if ($rejSusResult && mysqli_num_rows($rejSusResult) > 0) {
                                    // Fetch all search results
                                    $allrejSus = mysqli_fetch_all($rejSusResult, MYSQLI_ASSOC);

                                    // Calculate pagination parameters for search results
                                    $total_items_rej = count($allrejSus);
                                    ...
                                    // Slice the search results to display the current page
                                    $rejSus = array_slice($allrejSus, $start_index_rej, $items_per_page_rej);
                                }
                                ?>
                                <div class="tab-pane <?= ($active_tab == 3) ? ' active show' : '' ?>" id="rejectedRabid">
                                    <div class="table">
                                        <div class="card-body">
                                            <label for="rejPetSrch" class="form-label"></label>
                                            <form method="POST" action="dashboardRabidCases.php?active-tab=3">
                                                <div class="input-group mb-3">
                                            ...
                                            <div class=table-responsive>
                                                <table class="table text-center table-bordered table-hover" style="width:100%">
                                                    ...
                                                    <tbody id="valid-r">
                                                        <?php
                                                        // $case = new Cases();
                                                        // $items_per_page_rejectedRabid = 8; // Number of items per page

                                                        // // Get total number of rejected suspected rabid cases
                                                        // $total_items_rejectedRabid = count($case->getRejectedSus($brgyID));
                                                        // $total_pages_rejectedRabid = ceil($total_items_rejectedRabid / $items_per_page_rejectedRabid);

                                                        // // Initialize $page_rejectedRabid and set it based on your requirements
                                                        // $page_rejectedRabid = isset($_GET['page']) ? $_GET['page'] : 1;

                                                        // // Calculate the starting index for the current page
                                                        // $start_index_rejectedRabid = ($page_rejectedRabid - 1) * $items_per_page_rejectedRabid;

                                                        // // Get the data for the current page
                                                        // $rejSus = array_slice($case->getRejectedSus($brgyID), $start_index_rejectedRabid, $items_per_page_rejectedRabid);
                                                        if (empty($rejSus)) {
                                                            echo '<tr><td colspan="10">No data found.</td></tr>';
                                                        } else {
                                                            foreach ($rejSus as $row) {
                                                                echo '<tr class="text-center">';
                                                                echo '<td>' . $row['name'] . '</td>';
                                                                echo '<td>' . (!empty($row['pname']) ? $row['pname'] : 'Unknown') . '</td>';
                                                                echo '<td>' . ($row['petType'] == 0 ? 'Dog' : 'Cat') . '</td>';
                                                ...
                                                <ul class="pagination">
                                                    <?php for ($n = 1; $n <= $total_pages_rej; $n++) { ?>
                                                        <li class="page-item <?= ($n == $page_rej) ? 'active' : '' ?>">
                                                            <a class="page-link" href="./dashboardRabidCases.php?active-tab=3&page_rej=<?= $n ?>&rejSearch=<?php echo urlencode($search_rej); ?>"><?= $n ?></a>
                                                        </li>
                                                    <?php } ?>
                                                </ul>
                                            </div>
