...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    if (isset($_GET["caseID"])) {
        $caseID = $_GET["caseID"];
        $page_valid_bite = isset($_POST['page_valid']) ? $_POST['page_valid'] : 1;
        // var_dump($page_valid_bite);
        $case = new Cases();
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $markerCode .= "marker$geoID.bindPopup('Name: $pname<br>Barangay: $barangay <br><form method=\"POST\" action=\"process_updateBiteCaseLocation.php\"><input type=\"hidden\" name=\"page_valid\" value=\"$page_valid_bite\"><input type=\"hidden\" name=\"caseID\" value=\"$caseID\"><button type=\"submit\" name=\"geoID\" value=\"$geoID\" class=\"btn btn-primary\">Update Location</button></form>').openPopup();\n";
            }
            // echo $markerCode;
        }
    } else {
                    ...
                    <div class="col-md-8 p-1 mt-2 my-auto mx-auto">
                        <div class="container mt-2 p-3">
                            <div class="col-md-auto">
                                <a href="./dashboardBiteCases.php?active-tab=2&page_valid=<?php echo ($page_valid_bite); ?>" class="btn btn-lg align-item-start">
                                    <i class="bi bi-arrow-left"></i>
                                </a>
                            </div>
                            <div>
                                <h2 class="mt-3 mb-4">Bites Location:</h2>
                                <div id="map"></div>
                            </div>
